'''implementation of kmeans on 2d data.
no implementation of efficency improvements such as yinyang or initialisations.
'''
...
def kmeans(df, n, max_rounds=100, plot_results=False):
    ...
    centroids = \
        [(df.x.max() * np.random.random(), df.y.max() * np.random.random()) for i in range(n)]
    ...
    while label_changes != 0 and max_rounds != 0:
        ...
        if plot_results and (label_changes == 0 or round_counter == 0):
            ...
            print('Number of clusters left: ', \
                  len([c for c in centroids if c[0] == c[0] and c[1] == c[1]]))
...
df_test = create_test_data(N)

df_test, centroids_test = kmeans(df_test, N, plot_results=True)
